...
def test_order_book_sides_sorted_by_price() -> None:
    bids_side = _construct_order_book_side(entropy.Side.BUY, 5)
    asks_side = _construct_order_book_side(entropy.Side.SELL, 5)
...
def test_orderbook_top_bids_and_asks() -> None:
    bids = _construct_order_book_side(entropy.Side.BUY, 7)
    asks = _construct_order_book_side(entropy.Side.SELL, 7)
    orderBook: entropy.OrderBook = _construct_order_book(bids=bids, asks=asks)

    assert _get_order(bids, -1) == orderBook.top_bid
    assert _get_order(asks) == orderBook.top_ask
    # update orderbook
    update_bids = _construct_order_book_side(entropy.Side.BUY, 5)
    update_asks = _construct_order_book_side(entropy.Side.SELL, 5)
...
def test_orderbook_spread() -> None:
    ...
    asks = _construct_order_book_side(entropy.Side.SELL, 7)
    # None's in book
    orderBook: entropy.OrderBook = _construct_order_book(bids=bids, asks=[])
...
def _construct_order_book(
    bids: typing.Sequence[entropy.Order], asks: typing.Sequence[entropy.Order]
) -> entropy.OrderBook:
    # construct orderbook
    return entropy.OrderBook(
        symbol="TEST",
        lot_size_converter=entropy.NullLotSizeConverter(),
        bids=bids,
        asks=asks,
    )


def _get_order(orders: typing.Sequence[entropy.Order], index: int = 0) -> entropy.Order:
