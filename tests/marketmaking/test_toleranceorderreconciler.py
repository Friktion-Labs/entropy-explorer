...
from entropy.marketmaking.toleranceorderreconciler import ToleranceOrderReconciler
...
def test_buy_does_not_match_sell() -> None:
    ...
    existing = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal(1), quantity=Decimal(10)
    )
    desired = entropy.Order.from_values(
        entropy.Side.SELL, price=Decimal(1), quantity=Decimal(10)
    )
...
def test_quantity_within_positive_tolerance_matches() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal(1), quantity=Decimal("10.009")
    )
...
def test_quantity_positive_tolerance_boundary_matches() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal(1), quantity=Decimal("10.01")
    )
...
def test_quantity_outside_positive_tolerance_no_match() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal(1), quantity=Decimal("10.011")
    )
...
def test_quantity_within_negative_tolerance_matches() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal(1), quantity=Decimal("9.991")
    )
...
def test_quantity_negative_tolerance_boundary_matches() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal(1), quantity=Decimal("9.99")
    )
...
def test_quantity_outside_negative_tolerance_no_match() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal(1), quantity=Decimal("9.989")
    )
...
def test_price_within_positive_tolerance_matches() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal("1.0009"), quantity=Decimal(10)
    )
...
def test_price_positive_tolerance_boundary_matches() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal("1.001"), quantity=Decimal(10)
    )
...
def test_price_outside_positive_tolerance_no_match() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal("1.0011"), quantity=Decimal(10)
    )
...
def test_price_within_negative_tolerance_matches() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal("0.9991"), quantity=Decimal(10)
    )
...
def test_price_negative_tolerance_boundary_matches() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal("0.999"), quantity=Decimal(10)
    )
...
def test_price_outside_negative_tolerance_no_match() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal("0.9989"), quantity=Decimal(10)
    )
...
def test_time_in_force_early_match() -> None:
    ...
    desired = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal(1), quantity=Decimal(10), expiration=now
    )
...
def test_reconcile_no_acceptable_orders() -> None:
    existing = [
        entropy.Order.from_values(
            entropy.Side.BUY, price=Decimal(99), quantity=Decimal(10)
        ),
        entropy.Order.from_values(
            entropy.Side.SELL, price=Decimal(101), quantity=Decimal(10)
        ),
    ]
    desired = [
        entropy.Order.from_values(
            entropy.Side.BUY, price=Decimal(100), quantity=Decimal(10)
        ),
        entropy.Order.from_values(
            entropy.Side.SELL, price=Decimal(102), quantity=Decimal(10)
        ),
    ]
...
def test_reconcile_all_acceptable_orders() -> None:
    ...
    desired = [
        entropy.Order.from_values(
            entropy.Side.BUY, price=Decimal("99.01"), quantity=Decimal(10)
        ),
        entropy.Order.from_values(
            entropy.Side.SELL, price=Decimal("101.01"), quantity=Decimal(10)
        ),
    ]
    model_state = fake_model_state()
    actual = ToleranceOrderReconciler(
...
def test_reconcile_different_list_sizes_orders() -> None:
    existing = [
        entropy.Order.from_values(
            entropy.Side.BUY, price=Decimal(98), quantity=Decimal(20)
        ),
        entropy.Order.from_values(
            entropy.Side.BUY, price=Decimal(99), quantity=Decimal(10)
        ),
        ...
        entropy.Order.from_values(
            entropy.Side.SELL, price=Decimal(102), quantity=Decimal(20)
        ),
    ]
    desired = [
        entropy.Order.from_values(
...
def test_reconcile_two_acceptable_two_unacceptable_orders() -> None:
    ...
    desired = [
        entropy.Order.from_values(
            entropy.Side.BUY, price=Decimal("98.1"), quantity=Decimal(20)
        ),
        entropy.Order.from_values(
            entropy.Side.BUY, price=Decimal("99.01"), quantity=Decimal(10)
        ),
        ...
        entropy.Order.from_values(
            entropy.Side.SELL, price=Decimal("102.11"), quantity=Decimal(20)
        ),
    ]
    model_state = fake_model_state()
    actual = ToleranceOrderReconciler(
