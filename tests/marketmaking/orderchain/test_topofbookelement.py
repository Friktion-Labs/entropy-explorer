...
from entropy.marketmaking.orderchain.topofbookelement import TopOfBookElement
...
def test_top_check_ignores_own_orders_updated() -> None:
    ...
    bids: typing.Sequence[entropy.Order] = [
        fake_order(
            price=Decimal(78), quantity=Decimal(1), side=entropy.Side.BUY
    ...
    asks: typing.Sequence[entropy.Order] = [
        fake_order(
            price=Decimal(82), quantity=Decimal(3), side=entropy.Side.SELL
        ).with_update(owner=order_owner),
        fake_order(price=Decimal(83), quantity=Decimal(1), side=entropy.Side.SELL),
        fake_order(price=Decimal(84), quantity=Decimal(1), side=entropy.Side.SELL),
        fake_order(
    ...
    buy: entropy.Order = fake_order(
        price=Decimal(75), quantity=Decimal(6), side=entropy.Side.BUY
    )
