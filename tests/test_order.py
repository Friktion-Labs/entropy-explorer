...
def __build_initial_order() -> entropy.Order:
    owner = fake_seeded_public_key("owner")
    now = entropy.local_now()
    timestamp = now - timedelta(seconds=20)
    initial = entropy.Order.from_values(
        id=5,
        client_id=8,
        owner=owner,
        ...
        price=Decimal(88),
        quantity=Decimal(15),
        order_type=entropy.OrderType.POST_ONLY_SLIDE,
        reduce_only=True,
        timestamp=timestamp,
        expiration=now,
        match_limit=6,
    ...
    assert initial.side == entropy.Side.SELL
    assert initial.price == 88
    assert initial.quantity == 15
    assert initial.order_type == entropy.OrderType.POST_ONLY_SLIDE
...
def test_from_values_defaults() -> None:
    actual = entropy.Order.from_values(
        entropy.Side.BUY, price=Decimal(98), quantity=Decimal(20)
    )

    assert actual.id == 0
    assert actual.client_id == 0
    assert actual.owner == entropy.SYSTEM_PROGRAM_ADDRESS
    assert actual.side == entropy.Side.BUY
    assert actual.price == 98
    assert actual.quantity == 20
    assert actual.order_type == entropy.OrderType.UNKNOWN
    assert not actual.reduce_only
    assert actual.timestamp is None
    assert actual.expiration == entropy.Order.NoExpiration
    assert actual.match_limit == entropy.Order.DefaultMatchLimit


def test_from_values_all() -> None:
    owner = fake_seeded_public_key("owner")
    expiration = entropy.local_now()
    ...
    assert actual.side == entropy.Side.SELL
    assert actual.price == 88
    assert actual.quantity == 15
    assert actual.order_type == entropy.OrderType.POST_ONLY_SLIDE
...
def test_with_update_side() -> None:
    ...
    actual = initial.with_update(side=entropy.Side.BUY)
    assert actual.id == 5
    assert actual.client_id == 8
    assert actual.owner == initial_owner
    assert actual.side == entropy.Side.BUY  # Changed!
    assert actual.price == 88
    assert actual.quantity == 15
    assert actual.order_type == entropy.OrderType.POST_ONLY_SLIDE
    assert actual.reduce_only
...
def test_with_update_order_type() -> None:
    ...
    actual = initial.with_update(order_type=entropy.OrderType.MARKET)
    assert actual.id == 5
    assert actual.client_id == 8
    assert actual.owner == initial_owner
    assert actual.side == entropy.Side.SELL
    assert actual.price == 88
    assert actual.quantity == 15
    assert actual.order_type == entropy.OrderType.MARKET  # Changed!
    assert actual.reduce_only
    assert actual.timestamp == initial_timestamp
    assert actual.expiration == initial_expiration
    assert actual.match_limit == 6
...
def test_with_update_timestamp() -> None:
    ...
    updated_timestamp = entropy.local_now() - timedelta(seconds=10)
...
def test_with_update_expiration() -> None:
    ...
    updated_expiration = entropy.local_now() + timedelta(seconds=10)
...
def test_with_update_all() -> None:
    initial = __build_initial_order()
    updated_owner = fake_seeded_public_key("new owner")
    updated_timestamp = entropy.local_now()
    ...
    assert actual.order_type == entropy.OrderType.POST_ONLY
    assert not actual.reduce_only
...
def test_order_expired() -> None:
    actual = entropy.Order.from_values(
        ...
        price=Decimal(10),
        quantity=Decimal(20),
        order_type=entropy.OrderType.LIMIT,
        expiration=entropy.utc_now() - timedelta(seconds=1),
    )

    assert actual.is_expired_at(entropy.utc_now())
...
def test_order_not_expired() -> None:
    actual = entropy.Order.from_values(
        ...
        quantity=Decimal(20),
        order_type=entropy.OrderType.LIMIT,
        expiration=entropy.utc_now() + timedelta(seconds=5),
    )

    assert not actual.is_expired_at(entropy.utc_now())
...
def test_client_id_set_properly() -> None:
    ...
    explicit_client_id = entropy.Order.from_values(
        side=entropy.Side.BUY,
        price=Decimal(10),
        quantity=Decimal(20),
        order_type=entropy.OrderType.LIMIT,
