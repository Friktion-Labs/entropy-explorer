...
def test_constructor() -> None:
    ...
    actual = entropy.AccountInfo(address, executable, lamports, owner, rent_epoch, data)
...
def test_split_list_into_chunks() -> None:
    ...
    split_3 = entropy.AccountInfo._split_list_into_chunks(list_to_split, 3)
    assert len(split_3) == 4
    assert split_3[0] == ["a", "b", "c"]
    assert split_3[1] == ["d", "e", "f"]
    assert split_3[2] == ["g", "h", "i"]
    assert split_3[3] == ["j"]
    split_2 = entropy.AccountInfo._split_list_into_chunks(list_to_split, 2)
    ...
    split_20 = entropy.AccountInfo._split_list_into_chunks(list_to_split, 20)
