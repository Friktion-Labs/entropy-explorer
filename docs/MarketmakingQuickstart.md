...
Our server needs at least one file - the `id.json` that holds the secret key - but it can be useful to keep all entropy-explorer things together, away from any other files you may have.

So what we’ll do is create a directory called `entropy-explorer` in your home directory, put the `id.json` file there, and leave it up to you if you want to put logfiles or anything else in there too.

Run the following commands to set up the necessary directories and files:

```
# mkdir ~/entropy-explorer
# touch ~/entropy-explorer/id.json
# chmod 600 ~/entropy-explorer/id.json
```
...
```
# docker pull opinionatedgeek/entropy-explorer-v3:latest
```

You should see output like the following appear as the files are downloaded:

```
Unable to find image 'opinionatedgeek/entropy-explorer-v3:latest' locally
latest: Pulling from opinionatedgeek/entropy-explorer-v3
...
Status: Downloaded newer image for opinionatedgeek/entropy-explorer-v3:latest
```
...
```
# entropy-explorer generate-keypair --filename /app/id.json --overwrite
```

(/app/id.json is not a typo in the command - it’s the path to the ID file in the docker container’s context - it’s mapped to the ~/entropy-explorer/id.json file.)
...
(Nearly) all `entropy-explorer` commands take the following general parameters:

- `--cluster-name CLUSTER_NAME` - Solana RPC cluster name
- `--cluster-url CLUSTER_URL` - Solana RPC cluster URL
- `--group-name GROUP_NAME` - Entropy group name
- `--group-address GROUP_ADDRESS` - Entropy group address
- `--entropy-program-address MANGO_PROGRAM_ADDRESS` - Entropy program address
...
```
# entropy-explorer airdrop --symbol SOL --quantity 1 --url devnet
```
...
```
# entropy-explorer show-account-balances --cluster-name devnet
...
2021-08-27 19:16:34 ⓘ root         Context: « 𝙲𝚘𝚗𝚝𝚎𝚡𝚝 'Entropy Explorer':
    Cluster Name: devnet
    Cluster URL: https://entropy.devnet.rpcpool.com
    Group Name: devnet.2
    Group Address: Ec2enZyoC4nGpEfu2sUNAa2nUGJHWxoUWYSEJ2hNTWTA
    Entropy Program Address: 4skJ85cdxQAFVKbcGgfun8iZPL7BadVYXG3kGEGkufqA
...
Now let’s set up the Entropy Account. The Entropy Account is associated with a Entropy Group, and it holds your tokens and allows you to trade that Group’s markets using margin.

To create and initialise your Entropy Account, run:

```
# entropy-explorer ensure-account --cluster-name devnet --wait
...
```
# entropy-explorer airdrop --symbol USDC --quantity 10000 --faucet B87AhxX6BkBsj3hnyHzcerX2WxPoACC7ZyDr8E7H9geN --cluster-name devnet
```
...
Now that you have some devnet USDC in your wallet, it’s time to move it into your Entropy Account.
...
```
# entropy-explorer deposit --symbol USDC --quantity 10000 --cluster-name devnet
...
- `--group` The name of the Entropy Group you want to use. (Optional - will use the default group in the `ids.json` file in the container if none is specified.)
